...
for i, url in enumerate(urls):
    ...
    product_data.append(
        [f"Product_{i}", "Product", name, price, ' '.join(description), ' '.join(content)])

    print(f"Downloading Product_{i}")
    ...
    for j, img_url in enumerate(product_images):
        ...
        img_filename = f"{base_path}/{j+1}.jpg"
        try:
